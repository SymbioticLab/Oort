...
def draw_rs(clientSampleList, allSamples, totalClients, figCaption="diffall.pdf"):
    ...
    numOfSamples = [i for i in range(int(totalClients*0.01), int(totalClients * 0.5), int(totalClients*0.02))]
    ...
    for numOfSample in numOfSamples:
        random.shuffle(clientSampleList)
        nsubsetSamples = normalizeList(pickSubset(clientSampleList, numOfSample))
        distances.append(measureDistance(nsubsetSamples, nallSamples))

    return distances, array(numOfSamples)/float(totalClients)
...
# with open('divProfile', 'w') as fout:
#     dbs = [dE, dB, dO, dQ]
#     for item in dbs:
#         fout.writelines('\t'.join([str(x) for x in item]) + '\t')

res = []
with open('divProfile', 'r') as fin:
    lines = fin.readlines()
    for line in lines:
        print ('...')
        res.append([float(x) for x in line.strip().split()])
plot_cdf(res, ['email', 'blog', 'openImg', 'quickDraw'], "Individual Data Divergence to Overall", "CDF", "cdiffall.pdf")
