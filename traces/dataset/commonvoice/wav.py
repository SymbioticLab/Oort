...
parser.add_argument("--target-dir", default='/mnt/voice/english/', type=str, help="Directory to store the dataset.")
...
parser.add_argument('--files-to-process', default="train.tsv,test.tsv,validated.tsv,other.tsv",
...
def convert_to_wav(csv_file, target_dir):
    ...
    """
    wav_dir = os.path.join(target_dir, 'wav_3/')
    txt_dir = os.path.join(target_dir, 'txt_3/')
    ...
    def process(x):
        ...
        cmd = "sox {} -r {} -b 16 -c 1 {}".format(
            os.path.join(path_to_data, 'clips', file_path),
            args.sample_rate,
    ...
    for item in reader:
        ...
        item = item[0].split('\t')
        data.append((item[1], item[2])) # = [(row['path'], row['sentence']) for row in reader]
...
def main():
    ...
    for csv_file in args.files_to_process.split(','):
        print(csv_file)
        convert_to_wav(os.path.join(target_dir, csv_file), target_dir)

    #print('Creating manifests...')
    # for csv_file in args.files_to_process.split(','):
    #     print(os.path.join(target_dir, os.path.splitext(csv_file)[0]))
    #create_manifest(os.path.join(target_dir, 'train'), #os.path.splitext(csv_file)[0]),
    #                'train_manifest.csv',
    #                args.min_duration,
    #                args.max_duration)
